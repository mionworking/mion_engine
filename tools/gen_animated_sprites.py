...
def _draw_shadow(buf, W, H, ox, oy, fw, fh, pal):
    ...
    for dy in range(-2, 3):
        for dx in range(-7, 8):
            if (dx / 7) ** 2 + (dy / 2) ** 2 <= 1.0:
                a = int(pal["shadow"][3] * (1.0 - abs(dx) / 8.0))
...
def _body_south(buf, W, H, ox, oy, fw, fh, pal, bob, leg_phase, arm_swing):
    ...
    if style == "orc":
        ...
        # Skeleton: fino, costelas visíveis
        w = 3
        lleg_y = bob + int(3 * math.sin(leg_phase))
        rleg_y = bob + int(3 * math.sin(leg_phase + math.pi))
...
EXTRA_ROWS = {"s": [1], "n": [5], "e": [4, 6, 7]}


def render_sheet(pal: dict, weapon_fn) -> bytes:
    buf = new_buf(SW, SH)
    ...
    for dir_name, base_row in DIR_ROWS.items():
        for row in [base_row] + EXTRA_ROWS[dir_name]:
            ...
            for f in range(6):
                ...
                _draw_shadow(buf, SW, SH, ox, oy, FW, FH, pal)
                if dir_name == "s":
                    _body_south(buf, SW, SH, ox, oy, FW, FH, pal, bob, 0.0, 0.3 * math.sin(t * math.pi))
                    weapon_fn(buf, SW, SH, ox, oy, FW, FH, pal, bob, t)
                elif dir_name == "n":
                    _body_north(buf, SW, SH, ox, oy, FW, FH, pal, bob, 0.0, 0.3 * math.sin(t * math.pi))
                    weapon_fn(buf, SW, SH, ox, oy, FW, FH, pal, bob, t)
                else:
                    _body_east(buf, SW, SH, ox, oy, FW, FH, pal, bob, 0.0, 0.3 * math.sin(t * math.pi))
                    _weapon_east(buf, SW, SH, ox, oy, FW, FH, pal, bob, t)
...
def main() -> int:
    ...
    for spec in SPRITES:
        ...
        weapon_fn = _staff_south if spec["weapon"] == "staff" else _weapon_south
        rgba = render_sheet(pal, weapon_fn)
