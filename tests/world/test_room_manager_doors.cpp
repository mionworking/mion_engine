...
static void room_manager_build_room_non_boss_has_forward_and_town_doors() {
    ...
    EXPECT_TRUE(room.doors[0].target_scene_id.empty());
    EXPECT_EQ(room.doors[1].target_scene_id, WorldLayoutId::kTown);
}
...
static void room_manager_build_room_boss_has_only_town_exit_door() {
    ...
    EXPECT_EQ(room.doors[0].target_scene_id, WorldLayoutId::kTown);
}
